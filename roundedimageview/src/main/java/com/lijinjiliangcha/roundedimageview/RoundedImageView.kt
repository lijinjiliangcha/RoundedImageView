...
//    }
    ...
    private fun onArc(w: Int, h: Int) {
        ...
        path.arcTo(ltF, 180f, 90f, false)
        //右上角
        val rtF = RectF(w.toFloat() - r, 0f, w.toFloat(), r)
        path.arcTo(rtF, 270f, 90f, false)
        //右下角
        val rbF = RectF(w.toFloat() - r, h.toFloat() - r, w.toFloat(), h.toFloat())
        path.arcTo(rbF, 0f, 90f, false)
        //左下角
        val lbF = RectF(0f, h.toFloat() - r, r, h.toFloat())
        path.arcTo(lbF, 90f, 90f, false)
        createClipBitmap(w, h, path)
    }
    ...
    private fun createClipBitmap(w: Int, h: Int, path: Path) {
        rect = Rect(0, 0, w, h)
